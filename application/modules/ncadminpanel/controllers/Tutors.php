...
class Tutors extends CI_Controller {
	public function __construct() {
		...
		$this->module = "tutors";
		$this->module_label = get_label('tutor_manage_label');
		$this->module_labels =  get_label('tutor_manage_labels');
		$this->folder = "tutors/";
		$this->table = "tutors";
		$this->load->library ( 'common' );
		$this->primary_key = 'tutor_id';
	}
	...
	public function customeremail_exists() {
		$customer_email = $this->input->post ( 'tutor_email' );
		$edit_id = $this->input->post ( 'edit_id' );
		$user_arr = array();
		$where = array (
				'tutor_email' => trim ( $customer_email ),
		);
		if ($edit_id != "") {
			$where = array_merge ( $where, array (
					"tutor_id !=" => $edit_id,
		...
		$result = $this->Mydb->get_record ( 'tutor_id', $this->table, $where );
		if (! empty ( $result )) {
	...
	public function add() {
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			$this->form_validation->set_rules ( 'tutor_city', 'lang:tutor_city', 'required' );
			$this->form_validation->set_rules ( 'tutor_gender', 'lang:tutor_gender', 'required' );
			$this->form_validation->set_rules ( 'tutor_age', 'lang:tutor_age', 'required' );
			$this->form_validation->set_rules ( 'tutor_email', 'lang:tutor_email', 'required|callback_customeremail_exists' );
			$this->form_validation->set_rules ( 'tutor_mobile', 'lang:tutor_mobile', 'required' );
			$this->form_validation->set_rules ( 'tutor_city', 'lang:tutor_city', 'required' );
			$this->form_validation->set_rules ( 'tutor_location', 'lang:tutor_location', 'required' );
			$this->form_validation->set_rules ( 'tutor_address', 'lang:tutor_address', 'required' );
			$this->form_validation->set_rules ( 'tutor_qualification', 'lang:tutor_qualification', 'required' );
			$this->form_validation->set_rules ( 'tutor_experience', 'lang:tutor_experience', 'required' );
			$this->form_validation->set_rules ( 'tutor_subject', 'lang:tutor_subject', 'required' );
			$this->form_validation->set_rules ( 'tutor_type', 'lang:tutor_type', 'required' );
			$this->form_validation->set_rules ( 'status', 'lang:status', 'required' );
			
			if ($this->form_validation->run () == TRUE) {

				$insert_array = array (
						'tutor_name' => post_value ( 'tutor_name' ),
						'tutor_email' => post_value ( 'tutor_email' ),
						'tutor_age'=>post_value ( 'tutor_age' ),
						'tutor_gender'=>post_value ( 'tutor_gender' ),
						'tutor_city'=>post_value ( 'tutor_city' ),
						'tutor_address'=>post_value ( 'tutor_address' ),
						'tutor_mobile'=>post_value ( 'tutor_mobile' ),
						'tutor_qualification'=>post_value ( 'tutor_qualification' ),
						'tutor_experience'=>post_value ( 'tutor_experience' ),
						'tutor_subject'=>post_value ( 'tutor_subject' ),
						'tutor_location'=>post_value ( 'tutor_location' ),
						'tutor_description'=>post_value ( 'tutor_description' ),
						'tutor_type'=>post_value ( 'tutor_type' ),
						'tutor_time'=>post_value ( 'tutor_time' ),
						'tutor_class'=>post_value ( 'tutor_class' ),
						'tutor_pay'=>post_value ( 'tutor_pay' ),
						'tutor_status' => ($this->input->post ( 'status' ) == "A" ? 'A' : 'I'),
						'tutor_created_on' => current_date (),
						'tutor_created_ip' => get_ip () 
				);
	...
	function action() {
		...
		if ($postaction == 'Delete' && ! empty ( $ids )) {
			if (is_array ( $ids )) {
				$this->Mydb->delete_where_in($this->table,'tutor_id',$ids,array());
				$response ['msg'] = sprintf ( $this->lang->line ( 'success_message_delete' ), $this->module_label );
			} else {
				$this->Mydb->delete($this->table,array('tutor_id'=>$ids));
				$response ['msg'] = sprintf ( $this->lang->line ( 'success_message_delete' ), $this->module_label );
			}
			$response ['status'] = 'success';
