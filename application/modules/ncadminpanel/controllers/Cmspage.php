...
class Cmspage extends MX_Controller
{
	...
	function ajax_pagination($page = 0) {
		...
		$select_array = array ('cmspage_id ','cmspage_title','cmspage_description','cmspage_status','cmspage_slug','cmspage_status','cmspage_created_on','cmspage_updated_on');
	...
	public function edit($edit_id)
	{
		...
		if ($this->input->post ( 'action' ) == "edit") 
		{
			...
			if($this->form_validation->run($this)==TRUE)
			{
			  $data = array('cmspage_title' => post_value('cms_title'),'cmspage_slug' => make_slug(post_value('cms_slug'),$this->table,'cmspage_slug',array('cmspage_id !='.$id=>NULL)),'cmspage_description'=>$this->input->post('cms_description'),'cmspage_status' =>post_value('status'),'cmspage_updated_on' => current_date(),'cmspage_meta_title' =>post_value('cms_meta_title'),'cmspage_meta_description' =>post_value('cms_meta_description'),'cmspage_meta_keyword' =>post_value('cms_meta_keyword'),'cmspage_updated_by' => get_admin_id (),'cmspage_updated_ip' => get_ip () );
