...
Description		: Page contains email template add edit and delete functions..
...
class Emailtemplate extends CI_Controller {
	...
	function ajax_pagination($page = 0) {
	    ...
	    $total_rows = $this->Mydb->get_num_rows ( $this->primary_key, $this->table, $where, null, null, null, $like );
	...
	function action() {
		...
		if ($postaction == 'Delete' && ! empty ( $ids )) {
			...
			if (is_array ( $ids )) {
				...
				$this->Mydb->update_where_in ( $this->table, $this->primary_key, array ($ids), $update_values );
				$response ['msg'] = sprintf ( $this->lang->line ( 'success_message_delete' ), $this->module_label );
			}
			$response ['status'] = 'success';
