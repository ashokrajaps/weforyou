...
class Account extends MX_Controller
{
	...
	{   
		...
		{	
			...
			$check_result = $this->Mydb->get_record($select_array,$this->table,array('users_email = "'.$username.'" OR users_username = "'.$username.'"' =>NULL));
				...
				$password_verify = check_hash($password,$check_result['users_password']);
					...
					{ 
							...
							{		
									...
									{
										$data['redirect_url'] = base_url().'myaccount/myprofile';
									}		
					...
					else
					{
						/*invalid login detail*/
						$data['status'] = 'error';
						$data['msg'] = get_label ('flash_loginfailed');
						echo json_encode($data); exit;
					}	
			}
			else
			{
				/*invalid username*/
				$data['status'] = 'error';
				$data['msg'] = get_label ('no_records_found');
				echo json_encode($data); exit;
			}
		...
		if($login_with_type != '') 
		{
			...
			{	
				...
				if($check_user_status == "A")
				{
					...
					{ 	
						redirect(base_url().'myaccount/myprofile');
		...
		if($this->input->post('signin')=="Sign up")
		 {
			$this->form_validation
				...
				if($this->form_validation->run($this) == true)	
				{
					...
					/*generate activation key*/
					$activation_key = get_guid ( $this->table, 'users_activation_key', $customer_email );
					$user_type_check=trim(post_value('user_type'));
