...
class Account extends MX_Controller
{
	...
	{   
		...
		$login_before_url=$this->session->userdata('login_before_url');
		$this->session->set_userdata('login_before_url','');	
				...
				$password_verify = check_hash($password,$check_result['users_password']);
					...
					{ 
							...
							{		
									...
									elseif($login_before_url != "")
									{
										$data['redirect_url'] = $login_before_url;
									}								    
									else
									{
	...
	/*--------------------------------Login with Google plus----------------*/
			...
			$clientId = '157027549515-j1u7k8ob3nrr5ijhuuu61slj6943l0va.apps.googleusercontent.com';
			$clientSecret = 'xfIhh9_sAraccZM3KMP53lfb';			
	        $redirectUrl = base_url().'account/login';
...
/*
	...
	/*---------------------Twitter start----------------------------------*/
		...
		if($login_with_type != '') 
		{
			...
			{	
				...
				if($check_user_status == "A")
				{
					...
					$session_values = array( 'users_id' => $check_result['users_id'],'user_type'=>$check_result['user_type']);									
					$this->session->set_userdata($session_values);
					$user_details_check=$this->user_mandatory_fields_check($check_result['users_id']);
					if($user_details_check == '1')
					{ 	
			...
			else 
			{
				if ($login_username != '') 
				{
					...
					{   
								...
								$session_values = array('users_id' => $insert_id,'user_type'=>'Nonphysician');						
								$this->session->set_userdata($session_values);
		...
		if($login_before_url != "")
		{
		 redirect($login_before_url);//if login error redirect				
		}
		else
		{	
